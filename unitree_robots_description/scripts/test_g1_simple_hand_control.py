...
class G1SimpleHandControllerTest(Node):
    def __init__(self):
        super().__init__('g1_simple_hand_controller_test')
        ...
        self.left_simple_hand_client = ActionClient(
            self, FollowJointTrajectory, '/left_simple_hand_controller/follow_joint_trajectory'
        )
        
        self.right_simple_hand_client = ActionClient(
            self, FollowJointTrajectory, '/right_simple_hand_controller/follow_joint_trajectory'
        )
        
        # Joint names for each controller - matches your target joint list
        ...
        # Arms: kLeftShoulderPitch, kLeftShoulderRoll, kLeftShoulderYaw, kLeftElbow
        ...
        # Wrists: kLeftWristRoll, kLeftWristPitch, kLeftWristYaw
        ...
        # Simple hands: kLeftHandThumb0, kLeftHandThumb1, kLeftHandThumb2, 
        #               kLeftHandIndex0, kLeftHandIndex1, kLeftHandMiddle0, kLeftHandMiddle1
        self.left_simple_hand_joints = [
            'left_hand_thumb_0_joint', 'left_hand_thumb_1_joint', 'left_hand_thumb_2_joint',
            'left_hand_index_0_joint', 'left_hand_index_1_joint',
            'left_hand_middle_0_joint', 'left_hand_middle_1_joint'
        ]
        
        self.right_simple_hand_joints = [
        ...
        self.get_logger().info('G1 Simple Hand Controller Test Node started with action clients')
    ...
    def wait_for_servers(self):
        ...
        clients = [
            ...
            (self.left_simple_hand_client, 'left_simple_hand'),
            (self.right_simple_hand_client, 'right_simple_hand')
        ]
    ...
    def demo_sequence(self):
        """Demonstration sequence for G1 robot with simple hands (28 joints total)"""
        ...
        if self.step == 0:
            ...
            left_simple_hand_pos = [0.0] * len(self.left_simple_hand_joints)
            right_simple_hand_pos = [0.0] * len(self.right_simple_hand_joints)
            ...
            # Keep legs stable, focus on arm/hand movements - left hand makes fist
            ...
            # Left hand makes fist (curl all fingers), right hand stays open
            left_simple_hand_pos = [
                0.5, 0.5, 0.5,  # left thumb - curl
                -0.8, -0.8,     # left index - curl (negative values for left hand)
                -0.8, -0.8      # left middle - curl
            ]
            right_simple_hand_pos = [0.0] * len(self.right_simple_hand_joints)  # Right hand open
            
        elif self.step == 3:
            # Left hand points (index straight), right hand makes peace sign
            ...
            # Left hand points (index straight, others curl), right hand peace sign (index and middle straight)
            left_simple_hand_pos = [
                0.5, 0.5, 0.5,  # left thumb - curl
                0.0, 0.0,       # left index - straight (pointing)
                -0.8, -0.8      # left middle - curl
            ]
            right_simple_hand_pos = [
                0.5, 0.5, 0.5,  # right thumb - curl
        ...
        if self.step >= 0:
            ...
            left_simple_hand_goal = self.create_trajectory_goal(self.left_simple_hand_joints, left_simple_hand_pos)
            right_simple_hand_goal = self.create_trajectory_goal(self.right_simple_hand_joints, right_simple_hand_pos)
            ...
            self.send_goal_async(self.left_simple_hand_client, left_simple_hand_goal, 'left_simple_hand')
            self.send_goal_async(self.right_simple_hand_client, right_simple_hand_goal, 'right_simple_hand')
...
def main(args=None):
    ...
    controller_test = G1SimpleHandControllerTest()
